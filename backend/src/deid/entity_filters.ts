...
function isLikelyNotAPerson(text: string, fullText: string, position: number): boolean {
  ...
  for (const word of words) {
    ...
    if (medicalTerms.includes(word)) return true;
  }
  
  // Check if it's a medical term pattern (e.g., "Recurrent Ischemic Stroke")
  ...
  // Check if before context contains medical indicators
  if (medicalContextIndicators.some(indicator => beforeContext.includes(indicator))) {
    return true;
  }
  
  // Check if after context contains medical indicators
